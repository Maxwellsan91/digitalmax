...
export default function BlogPage({ locale = "pt" }: LocaleProps) {
  return (
    <>
      <SiteHeader locale={locale} />
      <main id="main-content" className="section-block section-pattern">
        <section className="section-shell">
          <h1 className="text-4xl font-semibold tracking-tight text-slate-950 sm:text-5xl">{t(locale, "Blog de marketing digital")}</h1>
          <p className="mt-5 max-w-3xl text-lg leading-relaxed text-slate-600">
            {t(locale, "Conteúdo educativo para negócios locais em Portugal que querem melhorar presença digital, aparecer melhor no Google e gerar mais pedidos de contacto.")}
          </p>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2">
            {blogPosts.map((post) => (
              <article key={post.slug} className="card-surface">
                ...
                <h2 className="mt-2 text-2xl font-semibold text-slate-950">{t(locale, post.title)}</h2>
